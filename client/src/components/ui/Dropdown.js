...
export default function Dropdown({name,list,bgColor,textColor,borderColor,...arg}) {
  ...
  return (
    <div onClick={()=>setUlState(!ulState)} className="py-1">
        <div>
            ...
            >
                {name}
                <svg
                aria-hidden="true" focusable="false" data-prefix="fas" data-icon="caret-down" className="w-2 ml-2" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"
                >
                <path
                    fill="currentColor"
            ...
            >
                {
                    list.map((item)=> (
                        <li key={item?.value}>
                            <span
                                className=" dropdown-item text-sm  z-50 py-2 px-4 font-normal block w-full whitespace-nowrap bg-transparent text-gray-700 hover:bg-gray-100 "
                                >{item?.text}</span>
                        </li>
                        // <div>{item?.value}</div>
                    ))
